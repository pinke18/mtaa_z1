...
#setup logging for diary
handler = logging.FileHandler("calls.log")
handler.setFormatter(logging.Formatter('%(asctime)s:%(levelname)s:%(message)s'))
logger = logging.getLogger("1st logger")
...
class UDPHandler(SocketServer.BaseRequestHandler):   
    ...
    def processRegister(self):
        ...
        if expires == 0:
            if fromm in registrar:
                del registrar[fromm]
                self.sendResponse("200 0K:Confirmed")
                return
        else:
            now = int(time.time())
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            ...
            if rx_register.search(request_uri):
                ...
                #print("invite")
                logger.info("%s is calling %s" % (self.getOrigin(), self.getDestination()))
                self.processInvite()
